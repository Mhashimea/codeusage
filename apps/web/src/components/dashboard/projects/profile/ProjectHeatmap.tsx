...
export function ProjectHeatmap({ projectSlug, initialData, initialYear }: ProjectHeatmapProps) {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      setLoading(true);
      try {
        const res = await fetch(`/api/v1/project-activity?slug=${encodeURIComponent(projectSlug)}&year=${year}`);
        if (res.ok) {
          const newData = await res.json();
          setData(newData);
        }
      } catch (error) {
        console.error("Failed to fetch project activity data:", error);
      } finally {
        setLoading(false);
      }
    };
    ...
    fetchData();
  }, [year, initialYear, initialData, projectSlug]);
