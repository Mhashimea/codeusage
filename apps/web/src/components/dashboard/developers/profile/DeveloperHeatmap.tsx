...
export function DeveloperHeatmap({ alias, initialData, initialYear }: DeveloperHeatmapProps) {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      setLoading(true);
      try {
        const res = await fetch(`/api/v1/developer-activity?alias=${encodeURIComponent(alias)}&year=${year}`);
        if (res.ok) {
          const newData = await res.json();
          setData(newData);
        }
      } catch (error) {
        console.error("Failed to fetch developer activity data:", error);
      } finally {
        setLoading(false);
      }
    };
    ...
    fetchData();
  }, [year, initialYear, initialData, alias]);
