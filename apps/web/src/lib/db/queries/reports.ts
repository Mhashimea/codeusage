...
export async function getWeeklySummary(
  ...
  const [summary] = await db
    .select({
      ...
      total_files: sql<string>`coalesce(sum(${tasks.files_changed}), 0)::bigint`,
  ...
  return {
    ...
    total_files: Number(summary.total_files),
...
export async function getMonthlyCostReport(
  ...
  return {
    ...
    total_cache_tokens: Number(summary.total_cache_tokens),
    total_files: Number(summary.total_files),
    unique_developers: summary.unique_developers,
    unique_projects: summary.unique_projects,
